...
(() => {
  ...
  const wrapperForHiding = document.querySelector<HTMLDivElement>('#wrapperForHiding');

  const hidedFromLocalstorage = JSON.parse(localStorage.getItem('allIsHided') || 'false');
  ...
  function syncWithWrapper() {
    ...
    if (hided)
      wrapperForHiding.classList.add('hidden');

    else
      wrapperForHiding.classList.remove('hidden');
  }
...
(async () => {
  const cardsContainer = document.querySelector<HTMLDivElement>('#usersCardsContainer');
  const searchInput = document.querySelector<HTMLInputElement>('#usersSearchInput');

  const userGeneratorResponse: UserGeneratorResponse = await fetch('https://randomuser.me/api?results=30').then(res => res.json());
  ...
  function renderUsersCards(users) {
    const cardsHTML = users.map((user) => {
      const contactsData = [
        ...
        { icon: 'ri:home-fill', value: `${user.location.country}, ${user.location.city}, ${user.location.street.name}, ${user.location.street.number}` },
      ];

      const contactsHTML = contactsData.map((item) => {
        return `
  ...
  <div class="userCard">
    ...
    />
    <p class="userName">
    <iconify-icon icon="flag:${user.nat.toLowerCase()}-4x3" class="icon"></iconify-icon>${user.name.title}. ${user.name.first} ${user.name.last}
    </p>
